...
sed -e 's/hour/'$HOUR'/g' -e 's/minute/'$TIME_CHUO'/g' -e 's/extime/'$CUO_MINUTE'/g' -e 's/ER_NAME/'$ERR_NAME'/g' -e 's/ER_SECRETE/'$NEW_SECRETE'/g' /root/task.sh > /data/vpn_user/$ERR_NAME"_task.sh"
if [[ ! -x $ERR_NAME"_task.sh" ]];then
